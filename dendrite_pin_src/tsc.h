...
/* Taken from Intel: https://www.intel.com/content/dam/www/public/us/en/documents/white-papers/ia-32-ia-64-benchmark-code-execution-paper.pdf
...
/*
   We use RDTSC and RDTSCP to benchmark function calls because they have low overhead (we just read a register)
   Per stack overflow benchmarks, we are looking at around 24 CPU cycles.
   The chief challenge with using RDTSC is that the TSC is not necessarily synchronized cores:
   - Not all OS's synchronize it cross core at boot (though most do)
   - There are ASM instructions that can adjust the TSC counter, though it is unclear why anyone would do this
   Generally it seems synchronized "enough" so as to not worry about the nanosecond-level variance...
   XXX If we get a bunch of jitter, we can RDTSCP first as well (instead of RDTSC) and compare the processor ID values,
   dropping any entries we have for cross-core recordings.
 */

/* 
   CPUID is a synchronizing instruction, which prevents instructions from being reordered before/after it.
...
/*
   ...
   Not a serializing instruction, but blocks until the prior instruction has executed.
